...
func NewLinkService(db *store.DB) *LinkService {
	return &LinkService{db: db}
}

// Create creates a new link
func (s *LinkService) Create(link *model.Link) error {
...
func (s *LinkService) GetByID(id string) (*model.Link, error) {
...
func (s *LinkService) GetTopClickedLinks(profileID string, limit int) ([]*model.Link, error) {
	query := `
		SELECT * FROM links
