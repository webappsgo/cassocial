...
func createTestUser(t *testing.T, db *store.DB, username, email string) string {
	...
	userID := generateUUID()
	// Argon2id hash for "ValidPass1" — use a bcrypt-like placeholder that passes DB
	// constraint; for auth-bypass tests the hash value is irrelevant.
	_, err := db.Exec(
...
func createTestProfile(t *testing.T, h *ProfileHandlers, userID, slug string) string {
	...
	if rr.Code != http.StatusCreated {
		t.Fatalf("createTestProfile: CreateProfile returned %d, want %d; body: %s", rr.Code, http.StatusCreated, rr.Body.String())
	}
	...
	id, _ := resp["id"].(string)
	if id == "" {
		t.Fatalf("createTestProfile: response missing id; got %v", resp)
	}
