...
func (s *AnalyticsService) TrackSession(session *models.AnalyticsSession) error {
	// Check if analytics is enabled
...
func (s *AnalyticsService) GetSummary(profileID string, startDate, endDate time.Time) (*models.AnalyticsSummary, error) {
	...
	for rows.Next() {
		var stat models.AnalyticsHourly
		if err := rows.Scan(
